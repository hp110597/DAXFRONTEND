  ...
  const handleOnSubmit = useCallback(
    (e) => {
      ...
      if (!Object.keys(newErrorMessage).length > 0) {
        ...
        fetch("http://128.199.251.244/api/order", {
          method: "POST", // or 'PUT'
          headers: {
  ...
  return (
    <div className="fixed inset-0 bg-gray-700 bg-opacity-50 flex justify-center items-center z-[1000]">
      <div className="bg-bgPrimary text-white rounded-lg inline-block w-11/12 md:max-w-2xl xl:max-w-3xl relative -z-10 box-border p-7 animate-dialogFadeIN overflow-hidden">
        ...
        <h3 className="text-center mb-10 text-lg tracking-widest">
        ...
        <div className="relative mb-6">
          ...
          <span className="inline-block p-4 bg-bgPrimary absolute top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2">
            hoặc
          </span>
        </div>
        ...
        <form onSubmit={handleOnSubmit}>
          ...
          <input
            type="submit"
            className="bg-primary px-16 py-2.5 rounded-2xl text-base font-avertaBold text-white cursor-pointer block mx-auto"
            value="Đặt hàng"
          />
        </form>
      </div>
