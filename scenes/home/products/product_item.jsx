...
const ProductItem = (props) => {
  ...
  return (
    <div className={`h-full w-full absolute 2xl:top-[120px] xl:top-[50px] lg:top-[100px] md:top-[50px] z-[2] ${productActive || prevProductActive ? '' : 'hidden'}
     ${productActive ? 'z-[5]' : ''}`}>
      <div className="xl:w-[1280px] xl:gap-x-20 lg:flex 2xl:w-[1400px] 3xl:w-[1700px]">
        ...
        <div className="text-white flex-shrink-1 grow mt-10 relative">
          <Text
            ...
            className={`font-avertaBold md:text-[48px] text-[30px] text-[#FDF8E2] md:mb-10 mb-5 ${prevProductActive ? 'animate-textAnimateFadeOut' : ''} lg:w-full md:w-[70%] md:mx-auto w-[87%] mx-auto tracking-widest 2xl:text-[60px] 3xl:text-[80px]`}
            text={product.name}
            duration='1.2s'
          />
          ...
          <ImageBackground
            ...
            alt='Next Button'
            className={`md:w-[32px] md:h-[32px] absolute md:top-1/2 w-[25px] h-[25px] top-1/2 right-5 3xl:right-16 2xl:right-0 xl:right-12 lg:right-32 md:right-16 cursor-pointer ${prevProductActive ? 'animate-imageFadeOut' : 'animate-imageFadeIn'}`}
            startX={`${prevProductActive ? '0%' : '500%'}`}
            endX={`${prevProductActive ? '200%' : '0%'}`}
            endY='-50%'
            onClickBtn={handleOnClick.bind({}, '-')}
          ...
          <div className="flex justify-end">
            <PrimaryButton
              className={`2xl:pl-8 2xl:pr-24 2xl:py-4 pl-6 pr-20 py-3 rounded-2xl bg-white/30 text-white cursor-pointer text-xl 2xl:text-2xl inline-block absolute top-[250px] md:top-[280px] lg:top-[310px] 3xl:top-[340px] transition hover:!scale-125 ${prevProductActive ? 'animate-fadeOut invisible' : 'animate-fadeIn'} ${productActive || prevProductActive ? '' : 'hidden'} xl:right-0 md:right-20 tracking-widest`}
